...
$stmtTheme = $pdo->prepare("SELECT theme FROM users WHERE id = ?");
$stmtTheme->execute([$user_id]);
$theme = $stmtTheme->fetchColumn() ?: 'dark';
            ...
            <div id="prompt-list">
                ...
                <?php else: ?>
                    <?php foreach ($user_prompts as $prompt): ?>
                        <button type="button" onclick="handleMenuClick()" class="prompt-item" 
...
if (resetBtn) {
    resetBtn.addEventListener('click', () => {
        ...
        showEmptyState();
        if (window.innerWidth <= 768) toggleMenu();
        // Correction: window.location.reload() au lieu de Window.onload()
...
async function sendMessage() {
    ...
    try {
        ...
        const res = await fetch('http://localhost:11434/v1/chat/completions', {
            ...
            body: JSON.stringify({ model: 'gemma3:12b', messages: payload })
        });
        const data = await res.json();
...
if (loadId) {
    (async function() {
        try {
            const res = await fetch(`backend/get.php?id=${loadId}`);
            const data = await res.json();
            if (data.success) {
                chatHistory = JSON.parse(data.messages);
                localStorage.setItem('chatHistory', JSON.stringify(chatHistory));
                ...
                document.getElementById('messages').innerHTML = '';
                initializeChat();
                window.history.replaceState({}, '', 'chatbot.php');
            }
        } catch (err) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (q) {
        const matchingPrompt = Array.from(promptItems).find(item => item.getAttribute('data-prompt') === q);
        if (matchingPrompt) {
            setUserInput(matchingPrompt.getAttribute('data-prompt'));
        } else {
            setUserInput(q);
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (params.get('auto') === '1' && params.get('q')) {
        const input = document.querySelector('#user-input');
        sendMessage();
        if (!input) return;
        input.value = decodeURIComponent(params.get('q'));
        setTimeout(() => {
            sendMessage();
        }, 300);
    }
